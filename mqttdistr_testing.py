...
def start_plusdistr(container):
    return net.addDocker(hostname=container.name, name=container.name, ip=container.address,
                         ...
                         cpuset_cpus=container.cpu,
                         environment={
                             "WS_PORT": str(8080+container.id),
                             "BROKER_PORT": str(1883+container.id),
                             "CLUSTER_SIZE": TOTAL_BROKERS,
                             "BROKER_NUM": container.id,
                             "IP_ADDR": container.address[:-3]
                         }
                         )
